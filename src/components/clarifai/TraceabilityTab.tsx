...
import { BrushCleaning, ChevronDown, ChevronsUpDown, ChevronUp, CircleCheck, EllipsisVertical, FunnelX, X } from 'lucide-react';
...
const [typeFilter, settypeFilter] = useState("All");
  const filteredRequirements = requirements.filter((req) => {
    ...
    const meetsSearch =
      ...
      req.user_story.toLowerCase().includes(searchText.toLowerCase()) || 
...
const selectedVisibleCount = filteredRequirements.filter(req => checkedMap[req.requirement_id]).length;
  ...
  <DropdownMenuTrigger asChild>
    <Button   variant="ghost" className="ml-auto text-sm text-black dark:text-white  border  border-dashed border-gray-500  px-3 py-2">
       Clear Options <EllipsisVertical></EllipsisVertical>
    </Button>
  </DropdownMenuTrigger>
...
<TableHead onClick={() => handleSort("confidence_score")} className="  px-4 py-2 cursor-pointer">
  <div className="inline-flex items-center gap-1 whitespace-nowrap">
    Confidence Level {sortConfig?.key === "confidence_score" ?(   sortConfig.direction === "asc" ? (
        <ChevronUp className="w-4 h-4" />
      ) : (
        <ChevronDown className="w-4 h-4" />
      )
...
<TableHead >
  <div className="flex items-center gap-2 ">
    ...
    <Checkbox className="bg-white border border-gray-500 dark:border dark:border-gray-900 dark:bg-[#414143] dark:text-black"
  checked={
