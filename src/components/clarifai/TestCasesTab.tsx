...
import { BrushCleaning, ChevronDown, ChevronsUpDown, ChevronUp, EllipsisVertical, FunnelX, X } from 'lucide-react';
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
goToTraceability: () => void;
  ...
  isLoading:boolean
  fulluserstoriesPayload:()=>void
} 
export default function TestCasesTab({ goToTraceability,testcaseData,fulltestcaseDataPayload,isLoading ,fulluserstoriesPayload}:TestCasesTabProps) {
...
const [showSuccess, setShowSuccess] = useState(true); // default true
...
const handleRegenerate = async () => {
  if(!fulluserstoriesPayload) return;
  ...
  try {
    const response = await fetch("http://127.0.0.1:8000/test_cases",{
      ...
      body: JSON.stringify(fulluserstoriesPayload),
    });
  ...
  <DropdownMenuTrigger asChild>
    <Button variant="ghost" className="ml-auto text-sm text-black dark:text-white  border border-dashed border-gray-200  px-3 py-2">
       Clear Options
  ...
  <DropdownMenuContent className="w-40  text-black dark:text-white border border-gray-700">
    ...
    <DropdownMenuItem onClick={() => {
     ...
     <BrushCleaning></BrushCleaning> Clear All
    </DropdownMenuItem>
  </DropdownMenuContent>
</DropdownMenu>
...
<Table>
  <TableHeader className="bg-white dark:bg-[#111111] text-black dark:text-white">
    <TableRow>
      <TableHead onClick={() => handleSort("test_id")} className="px-3 py-2 w-[150px] cursor-pointer">
        <div className="inline-flex items-center gap-1 whitespace-nowrap">
          REQ ID
          {sortConfig?.key === "test_id" ? (
      ...
      <TableHead onClick={() => handleSort("title")} className="px-3 py-2 w-[230px] cursor-pointer">
        <div className="inline-flex items-center gap-1 whitespace-nowrap">
          Title
      ...
      <TableHead onClick={() => handleSort("precondition")} className="px-3 py-2 w-[250px] cursor-pointer">
        <div className="inline-flex items-center gap-1 whitespace-nowrap">
          Precondition
          {sortConfig?.key === "precondition" ? (
            sortConfig.direction === "asc" ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />
          ) : (
            <ChevronsUpDown className="w-4 h-4" />
          )}
      ...
      <TableHead onClick={() => handleSort("priority")} className="px-3 py-2 w-[100px] cursor-pointer">
        <div className="inline-flex items-center gap-1 whitespace-nowrap">
          Priority
      ...
      <TableHead onClick={() => handleSort("tags")} className="px-3 py-2 w-[260px] cursor-pointer">
        <div className="inline-flex items-center gap-1 whitespace-nowrap">
          Tags
          {sortConfig?.key === "tags" ? (
            sortConfig.direction === "asc" ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />
      ...
      <TableHead className="px-3 py-2 w-[100px]"></TableHead>
    </TableRow>
  </TableHeader>
</Table>

<ScrollArea className="overflow-auto h-[225px] w-full border-t border-gray-700">
  <Table className="min-w-[1000px] text-sm text-left text-white">
    <TableBody>
      {sortedRequirements.map((req, index) => (
        ...
        >
          <TableCell className="px-3 py-2 w-[150px]">{req.test_id}</TableCell>

          <TableCell className="px-3 py-2 w-[230px] whitespace-normal break-words">
            {req.title}
          </TableCell>

          <TableCell className="px-3 py-2 w-[250px] whitespace-normal break-words">
            {req.precondition}
          </TableCell>

          <TableCell className="px-3 py-2 w-[100px]">
            <Badge
          ...
          <TableCell className="px-3 py-2 w-[260px] whitespace-normal break-words">
            <div className="whitespace-pre-line break-words">{req.tags.join(', ')}</div>
          </TableCell>
